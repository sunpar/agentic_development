...
- Implemented self-contained implementation-wave summaries with plan path/hash, selected task ids, wave statuses, per-task wave numbers, branches, worktrees, prompt paths, statuses, and errors.
...
- Implemented per-wave `--max-parallel` in `orchestrate_implementation_waves.py`: same-wave task workers run concurrently, git worktree creation stays serialized, merge gates run afterward in `integration_order`, and historical run reports preserve `--max-parallel` in failed-task resume commands.
- Implemented in `report_implementation_wave_runs.py`: aggregate reporting across multiple historical implementation-wave runs, including dry-run counts, selected waves, failed waves, task status totals, failed tasks, branches, worktrees, prompt paths, PR numbers, review-request counts, review-repair counts, merged task counts, merge log paths, and failed-task resume commands from `run-summary.json` plus `run-state.json` metadata or from `run-state.json` alone.
...
- Implemented bounded implementation task review repair in `orchestrate_implementation_waves.py`: unresolved review-thread gate failures can trigger Codex repair prompts, verification, commit/push, addressed thread resolution, fresh review requests, and merge-gate retry under `--review-repair-attempts`.
- Remaining follow-up: dogfood implementation-wave Codex/PR/review/merge automation on a real multi-task feature.
...
- Implemented in `report_codebase_review_runs.py`: aggregate reporting across multiple historical runs, including status totals, failed slices, PR counts and per-run branches, worktrees, PR numbers, review-request counts, merged-slice counts, and failed-slice resume commands that preserve saved setup, PR, review-request, and merge options from `run-summary.json` plus `run-state.json` metadata or from `run-state.json` alone.
